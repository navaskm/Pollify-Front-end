...
import { userProfileStyles as s } from "@/public/style/style";
import { PublicProfileData } from "@/utils/types";
...
export default function UserProfilePage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await api.get<PublicProfileData>(`/user/${username}`);
      setData(res.data);
      setMissing(false);
    } catch {
      setMissing(true);
  ...
  if (missing || !data) return <div className={s.errorContainer}>User not found.</div>;
