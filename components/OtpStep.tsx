...
import { AuthButton } from "./UIElements";
...
export default function OtpStep({
  ...
  const [left, setLeft] = useState(60); // 60 second timer
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if(axios.isAxiosError(error)){
        setError(error.response?.data?.message || error.response?.data?.msg || "Invalid or expired OTP");
      }
    } finally {
      setBusy(false);
    }
