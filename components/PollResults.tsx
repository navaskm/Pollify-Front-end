...
import { pollResultsStyles as s } from "@/public/style/style";
import { PollResultsProps, VersusBarProps, ResultBarProps } from "@/utils/types";
...
// this is the small helper component to get the result (output)
function ResultBar({ label, percent, highlight, winner, onClick }: ResultBarProps) {
...
// if the question type is yes/no the bar will be like a versus bar
// a small component for versus bar for only yes/no question
function VersusBar({ results, myVote, total, onUnvote }: VersusBarProps) {
...
export default function PollResults({ poll, onUnvote } : PollResultsProps) {
  ...
  const avg = total
    ? poll.results.reduce((a, r) => a + (r.star ?? 0) * r.count, 0) / total
    : 0;
